...
const now = new Date();
const[alertHours, alertMinutes] = alert.Medication_Time.split(':').map(Number);
const isNow = now  .getHours() === alertHours && now  .getMinutes() === alertMinutes;
...
const activeAlerts =
activeTab === 'upcoming' ? upcomingAlerts :
activeTab === 'pending' ? pendingAlerts :
activeTab === 'history' ? historyAlerts :
[];

const filteredAlerts = activeAlerts.filter(alert => {
if (activeTab === 'upcoming') return true;
if (activeTab === 'pending') return alert.status === 'pending';
if (activeTab === 'history') return alert.status === 'administered' || alert.status === 'cancelled';
return false;
});
...
if (alert) {
  ...
  setUpcomingAlerts(prev => prev.filter(a => a.schedule_id !== selectedCancelId));
  setPendingAlerts(prev => prev.filter(a => a.schedule_id !== selectedCancelId));
...
return ( <View style={styles.container}> <Sidebar />
  <View style={[styles.mainRow, { marginLeft: sidebarWidth }]}> <View style={styles.alertPanel}> <View style={styles.tabHeader}>
...
onPress={() => setActiveTab(tab)}
  >
{tab.charAt(0).toUpperCase() + tab.slice(1)} </Text>
))} </View>
          ...
          {sortedAlerts.length === 0 ? (
            ...
            sortedAlerts.map((alert) => {
              ...
              return (
                <TouchableOpacity
                  key={scheduleId}
                  style={[styles.alertItem, getCardStyle(alert.Medication_Time)]}
                  onPress={() => toggleExpand(scheduleId)}
                  activeOpacity={0.9}
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                  <Checkbox
                    status={checkedAlerts[scheduleId] ? 'checked' : 'unchecked'}
                    onPress={activeTab !== 'history' ? () => toggleCheckbox(scheduleId) : null}
                    color={checkedAlerts[scheduleId] ? '#333' : '#cccccc'}
                    style={styles.checkboxPosition}
                  />
                    <View style={{ marginLeft: 10, flex: 1 }}>
                      <Text style={[styles.alertText, { color: getTextColor(alert.Medication_Time) }]}>
                        Schedule ID: {alert.patientID} - {scheduleId}
                      </Text>
                      {isExpanded && (
                        <>
                          <Text style={[styles.alertText, { color: getTextColor(alert.Medication_Time) }]}>
                          ...
                          {activeTab !== 'history' && (
                            <View style={styles.buttonContainer}>
                              ...
                              <TouchableOpacity 
                              style={[styles.button, { backgroundColor: '#5c87b2' }]}
                              onPress={() => {
                                if (activeTab === 'upcoming') {
                              ...
                              <Text style={styles.buttonText}>Administered</Text>
                              </TouchableOpacity>
                            </View>
                          )}
                        </>
